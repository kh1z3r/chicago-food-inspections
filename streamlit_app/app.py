...
from utils import (
    apply_theme, eyebrow, star_rule, finding, load_csv, style_figure,
    NAVY_2, RED, FLAG_DIVERGING, _star_svg,
)
...
""",
    ...
    if flip is not None and geo is not None:
        ...
        fig = px.scatter_mapbox(
            ...
            size_max=26, color_continuous_scale=FLAG_DIVERGING,
            hover_name="Zip",
    ...
    st.markdown(
        """
Walk the audit in order from the sidebar:
...
nav = st.navigation([
    st.Page(overview, title="Overview", icon=":material/star:", default=True),
    st.Page("pages/1_Model_Performance.py", title="Model Performance", icon=":material/query_stats:"),
    st.Page("pages/2_What_Drives_Predictions.py", title="What Drives Predictions", icon=":material/insights:"),
    st.Page("pages/3_Neighborhood_Effect.py", title="Neighborhood Effect", icon=":material/map:"),
    st.Page("pages/4_Threshold_Tuning.py", title="Threshold Tuning", icon=":material/tune:"),
    st.Page("pages/5_Fairness_Audit.py", title="Fairness Audit", icon=":material/balance:"),
    st.Page("pages/6_Enforcement_Check.py", title="Enforcement Check", icon=":material/gavel:"),
    st.Page("pages/7_Conclusions.py", title="Conclusions", icon=":material/flag:"),
])
nav.run()
